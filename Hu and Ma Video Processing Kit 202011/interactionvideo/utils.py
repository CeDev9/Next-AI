...
def google_result_to_df(google_result):
    ...
    for each_text in google_result.results:
        ...
        # Add the time
        each_text_result_df['Onset'] = [each.start_time.seconds + \
                          each.start_time.nanos * 1e-9 for each in words_info]
        
        each_text_result_df['Offset'] = [each.end_time.seconds + \
                          each.end_time.nanos * 1e-9 for each in words_info]
